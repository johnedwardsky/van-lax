...
function randomize() {
    ...
    // Harmonic & Structured Mode Logic:
    // Favor small integers and simple fractions for harmonic resonance
    const harmonicValues = [1, 1.5, 2, 2.5, 3, 4, 5, 6, 8, 12];
    const pickHarmonic = () => {
        const base = harmonicValues[Math.floor(Math.random() * harmonicValues.length)];
        const sign = Math.random() > 0.5 ? 1 : -1;
        const drift = (Math.random() - 0.5) * 0.015; // Small drift for density without chaos
        return (base * sign) + drift;
    };

    targetParams.symmetry = Math.random() > 0.3 ? Math.floor(y(2, 6)) : 1; 
    targetParams.rotorRPM = (Math.random() - 0.5) * 2; // Slower, more controlled rotation
    targetParams.lrpm = pickHarmonic();
    targetParams.rrpm = pickHarmonic();
    
    targetParams.baseoffsx = y(-50, 50);
    targetParams.baseoffsy = y(-100, 100);
    targetParams.handdist = y(100, 250); 
    
    targetParams.larm1 = y(40, 120); 
    targetParams.rarm1 = y(40, 120); 
    targetParams.larm2 = y(150, 280); 
    targetParams.rarm2 = y(150, 280); 
    targetParams.rarmext = y(20, 80); 
    targetParams.larma = y(0, 360);
    
    targetParams.acceleration = Math.floor(y(150, 300)); 
    ...
    for (let i = 0; i < acceleration; i++) {
        ...
        if (dist > 0.1 && dist < L2 + R2 && dist > Math.abs(L2 - R2)) {
            ...
            for (let s = 0; s < symCount; s++) {
                ...
                ctx.strokeStyle = `rgba(${r}, ${g}, ${b}, 0.45)`; // Higher opacity for vibrancy
                ctx.lineWidth = 0.75; // Balanced width for clarity
